    ...
    <header className='sticky top-0 z-50 w-full bg-white'>
      ...
      <div className='border-b border-gray-100'>
        <div className='mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:h-[72px] lg:px-8'>
          ...
          <button
            type='button'
            className='inline-flex items-center justify-center rounded-md p-2 text-gray-700 transition-colors hover:bg-gray-100 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fcu-primary-900 lg:hidden'
                ...
                >
                  <Link
                    ...
                    className={`inline-flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
                      isOpen
                        ? 'bg-gray-50 text-fcu-primary-900'
          ...
          <div className='flex items-center gap-1 sm:gap-2'>
            ...
            {utilityNav?.secondaryAction?.url && (
              <a
                ...
                className='hidden rounded-md px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fcu-primary-900 md:inline-flex'
              >
            ...
            {utilityNav?.primaryAction?.url && (
              <Link
                ...
                className='inline-flex items-center rounded-full bg-fcu-primary-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-fcu-primary-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fcu-primary-900 focus-visible:ring-offset-2'
              >
                        ...
                        <li key={navLink._key}>
                          {isExternalLink(navLink) ? (
                            <a
                              ...
                              target='_blank'
                              rel='noopener noreferrer'
                              className='group inline-flex items-center gap-1.5 text-sm text-gray-600 transition-colors hover:text-fcu-primary-900'
                            ...
                            <Link
                              href={getHref(navLink)}
                              className='text-sm text-gray-600 transition-colors hover:text-fcu-primary-900'
                              onClick={() => setActiveMenu(null)}
                            >
                              {navLink.label}
                            </Link>
      ...
      >
        <nav
          className='h-full overflow-y-auto px-4 pb-24 pt-2'
          aria-label='Mobile navigation'
        >
          {mainNav.map((item) => {
            const hasMega = item.megaMenu && item.megaMenu.length > 0
            ...
            return (
              <div key={item._key} className='border-b border-gray-100'>
                <div className='flex items-center'>
                  <Link
                    href={item.url}
                    className='flex-1 py-3.5 text-base font-medium text-gray-900 transition-colors hover:text-fcu-primary-900'
                            ...
                            <li key={navLink._key}>
                              {isExternalLink(navLink) ? (
                                <a
                                  ...
                                  target='_blank'
                                  rel='noopener noreferrer'
                                  className='inline-flex items-center gap-1.5 text-sm text-gray-600 transition-colors hover:text-fcu-primary-900'
                                  onClick={() => setMobileOpen(false)}
                                >
                                  {navLink.label}
                                  <ExternalLink
          ...
          <div className='mt-6 space-y-3'>
            ...
            {utilityNav?.primaryAction?.url && (
              <Link
                href={utilityNav.primaryAction.url}
                className='flex w-full items-center justify-center rounded-lg bg-fcu-primary-900 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-fcu-primary-950'
                onClick={() => setMobileOpen(false)}
              >
                {utilityNav.primaryAction.label}
              </Link>
