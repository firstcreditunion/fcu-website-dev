    ...
    >
      <Subsection title="Primary Logo">
        <div className="grid gap-6">
          <div className="group relative flex items-center justify-center rounded-2xl border border-fcu-primary-100 bg-white p-12 transition-shadow hover:shadow-lg">
            <Image
              src="/fcu-logo.png"
            ...
            <Badge
              variant="outline"
              className="absolute top-3 left-3 border-fcu-primary-200 text-[10px] text-fcu-primary-500"
            >
              Primary Usage
            </Badge>
          </div>
      ...
      >
        <div className="flex items-center justify-center rounded-2xl border border-dashed border-fcu-primary-200 bg-fcu-primary-50/30 p-16">
          <div className="relative">
            {/* Clear space indicators */}
            <div className="absolute -inset-8 rounded-xl border-2 border-dashed border-fcu-secondary-400/40" />
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 -translate-y-3">
              <span className="rounded-full bg-fcu-secondary-500/10 px-2 py-0.5 text-[9px] font-bold text-fcu-secondary-700">
                1x
              </span>
            </div>
      ...
      >
        <div className="flex flex-wrap items-end gap-8 rounded-2xl border border-fcu-primary-100 bg-white p-8">
          <div className="text-center">
            ...
            />
            <p className="mt-3 text-xs font-medium text-fcu-primary-600">
              Digital minimum
            </p>
            <p className="text-[10px] text-fcu-primary-400">120px wide</p>
          </div>
          <div className="text-center">
            <Image
              src="/fcu-logo.png"
            ...
            <p className="text-[10px] text-fcu-primary-400">200px+ wide</p>
          </div>
        </div>
      </Subsection>
      ...
      <Subsection title="Usage Do's and Don'ts">
        <div className="grid gap-4 sm:grid-cols-2">
          {/* Do's */}
          <div className="rounded-2xl border border-fcu-secondary-200 bg-fcu-secondary-50/30 p-6">
            <div className="mb-4 flex items-center gap-2">
              <span className="flex size-6 items-center justify-center rounded-full bg-fcu-secondary-500 text-white">
                <Check className="size-3.5" />
              </span>
              <h4 className="text-sm font-bold text-fcu-secondary-800">Do</h4>
            </div>
            <ul className="space-y-2.5">
              {LOGO_DOS.map((item) => (
                <li
                  key={item}
                  className="flex items-start gap-2 text-xs leading-relaxed text-fcu-secondary-700"
                >
      ...
      <Subsection title="Favicon & App Icons">
        <div className="flex flex-wrap gap-6 rounded-2xl border border-fcu-primary-100 bg-white p-8">
          {[
            { size: '16×16', file: 'favicon-16x16.png', src: '/favicon-16x16.png' },
            { size: '32×32', file: 'favicon-32x32.png', src: '/favicon-32x32.png' },
            { size: '180×180', file: 'apple-touch-icon.png', src: '/apple-touch-icon.png' },
            { size: '192×192', file: 'android-chrome-192x192.png', src: '/android-chrome-192x192.png' },
            { size: '512×512', file: 'android-chrome-512x512.png', src: '/android-chrome-512x512.png' },
          ].map((icon) => (
            <div key={icon.file} className="text-center">
              <div className="mb-2 inline-flex items-center justify-center rounded-xl border border-fcu-primary-100 bg-fcu-primary-50/50 p-3">
                <Image
                  src={icon.src}
              ...
              <p className="text-[10px] font-bold text-fcu-primary-600">{icon.size}</p>
              <p className="text-[9px] text-fcu-primary-400">{icon.file}</p>
            </div>
          ))}
        </div>
